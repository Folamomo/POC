...
function show_with_mask(img, mask, size)
    ...
    subplot(3, 2, 3), imshow(norm1, []), title('+128');
    subplot(3, 2, 4), imshow(norm2, []), title('abs');
    subplot(3, 2, 5), imshow(uint8(img + norm1), []), title('original + mask + 128');  
    subplot(3, 2, 6), imshow(uint8(img + norm2), []), title('original + abs(mask)');  
end
